...
class CustomStylesEndpointController extends ControllerBase {
  ...
  public function customStyleTypeAll() {
    ...
    foreach ($custom_style_types = \Drupal::entityTypeManager()->getStorage('custom_style_type')->loadMultiple() as $type) {
   ...
   */
  private function getStyleChildren(\Drupal\Core\Entity\EntityStorageInterface $storage, $child_ids = []) {
    $children = [];
    if ($child_ids) {
  ...
  /**
   *
   * @param CustomStyleType $custom_style_type
   *
   * @return array
   */
  private function customStyleEntities(CustomStyleType $custom_style_type = NULL) {
    $data = [];
    try {
      $storage = \Drupal::service('entity.manager')->getStorage('cohesion_custom_style');
   ...
   *
   * @return CustomStyleType|NULL
   */
  private function customStyleTypeEntityById($id) {
    try {
      return \Drupal::service('entity.manager')->getStorage('custom_style_type')->load($id);
    } catch (\Drupal\Component\Plugin\Exception\PluginNotFoundException $ex) {
      watchdog_exception('cohesion', $ex);
    }
    return NULL;
